...
3) Main area: responsive grid of ~12 term cards (3 columns). Card anatomy: term name "按钮 Button" in bold, a soft indigo chat bubble with a colloquial Chinese sentence inside, bottom row with a single category tag chip + a star favorite icon.
4) First card is in hover state with subtle lift shadow and indigo border.
5) Above the grid: only a small result count; no extra level filter.
...
2) H1 "组件 Component" + a single category chip 前端.
...
7) Right side of hero: faint constellation line-art decoration.
