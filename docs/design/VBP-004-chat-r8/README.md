...
用户要求仅保留核心动画：移除可视箭头文字、工具描述、权限及结束提示、卡片下解释、历史摘要和 Chat 底部脚注；保留步骤标题、角色、对话、工具请求及结果。Chat 回复改为直接说明只收到文件名。箭头保留无障碍标签，动画时序不变。AGENTS.md 逐词条定制部分新增“互动演示保持极简”规则。

隔离工作区再次构建通过（含 TypeScript），浏览器检查读取步骤及 Chat 显示。DP 任务 9bd1709c-29d3-4e67-baa4-f6eb383721bc。本次仍为本地预览，不推送或部署。
