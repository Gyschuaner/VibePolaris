...
1. 先评审现有 72 条样稿，确定 VibePolaris 的最终文字密度和详情页字段。
2. 从 P0 中选出第二批 30 条，优先补编程基础、网络、移动端和 Agent 工程的代表词。
3. 为首批词建立 `related_slugs`，先形成小而清晰的星图关系，再继续铺量。
4. 通过审核后再写入 `content/zh/terms.json`；候选池继续作为选题源，不直接驱动线上页面。
