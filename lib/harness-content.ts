...
export const harnessSteps = [
  ...
  { title: "开放工具", summary: "读取反馈文件，把实际内容交回模型。工具权限限定为只读，不能修改原始反馈。", evidence: "read_file /data/feedback.md → 返回 12 条反馈", note: "工具让模型获取外部信息；工具能做什么，由运行环境和权限共同决定。" },
  { title: "控制循环", summary: "模型归纳后，检查发现少了一个来源编号。将缺项反馈给模型，补齐后再检查。", evidence: "第 1 轮：缺少 #08 → 第 2 轮：补齐引用", note: "最多 3 轮；达到验收标准就停止，超过预算或遇到无权操作则交回人处理。" },
  { title: "验收证据", summary: "检查 3 类要点、12 条反馈覆盖情况与原文编号，保留总结和执行轨迹。", evidence: "summary.md：3 类要点 · 12/12 覆盖 · 来源齐全", note: "示例检查通过，完成并可核对。真实项目仍需按任务标准判断内容是否正确。" },
] as const;
