...
桌面鼠标保留系统指针，并附带轻量主题色光晕：可点击内容悬停时收拢；正文、空白和控件按下时立即扩散并压缩光晕。按住主键拖动时，沿途不断泛起独立涟漪，松开停止发射，已有水波在 650ms 内自然消退。拖动选字正常，松开选区后隐藏光晕，保留水波余韵；输入框、滚动与星图画布隐藏全局光晕，触摸与减少动态偏好不启用。波纹固定复用16个节点，拖动按时间与距离限频；移动光晕按帧合并，不触发 React 渲染或空闲帧循环。验证见 [VBP-009 拖动水波](docs/design/VBP-009-drag-ripples/README.md)。
