...
桌面鼠标保留系统指针，并附带轻量主题色光晕：可点击内容悬停时收拢；正文、空白和控件按下时立即扩散并压缩光晕，松开恢复。扩散在 650ms 内结束，不被链接滚动或站内路由切换截断。输入、选字、拖动、滚动及星图画布隐藏全局光晕；触摸与减少动态偏好不启用。移动事件按帧合并，只更新独立图层，不触发 React 渲染或空闲帧循环。
