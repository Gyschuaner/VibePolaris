...
桌面鼠标保留系统指针和轻量主题色光晕，可点击内容悬停时收拢。按住主键拖动时，使用已确认的 A「水面包络」：最近 840ms 的轨迹融合为一条连续水痕，尾部渐细，松开后整片轮廓在 780ms 内舒展消散。原生选字保留，输入框与星图避让；滚动、离开窗口时清理效果，触摸与减少动态偏好不启用。固定一个 SVG 路径，轨迹采样按帧合并，并限制历史距离与点数，不触发 React 渲染或空闲帧循环。设计与接入记录见 [VBP-009 连续包络线](docs/design/VBP-009-envelope/README.md)。
