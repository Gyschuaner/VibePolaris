...
(function () {
  ...
  function clearMotion(detail) {
    if (!detail) return;
    if (detail._vpMotionTimer) clearTimeout(detail._vpMotionTimer);
    if (detail._vpMotionFrame) cancelAnimationFrame(detail._vpMotionFrame);
    detail._vpMotionTimer = 0;
    detail._vpMotionFrame = 0;
  }

  function conceal(domain, detail) {
    ...
    detail._vpMotionTimer = setTimeout(function () {
  ...
  function reveal(domain, detail) {
    ...
    detail._vpMotionFrame = requestAnimationFrame(function () {
      detail._vpMotionFrame = requestAnimationFrame(function () {
        detail.classList.remove('is-entering');
        detail._vpMotionFrame = 0;
      });
  ...
  function activate(target) {
    ...
    domains.forEach(function (domain) {
      ...
      if (active) reveal(domain, detail);
      else conceal(domain, detail);
    });
  }
...
window.VP_TERMS = [
  // 前端 4
  { zh: '组件', en: '组件', slug: 'component', cat: '前端', say: '就是页面里可以重复使用的那一块，比如导航栏、按钮这些。', alias: '控件 积木 模块 重复的部分' },
  { zh: '状态', en: 'State', slug: 'state', cat: '前端', say: '提交按钮点下去先显示保存中，成功或失败后再更新提示。', alias: '数据变化 界面跟着变' },
  { zh: '响应式布局', en: 'Responsive', slug: 'responsive', cat: '前端', say: '手机上打开别要左右滑，栏目要自己排成一列。', alias: '适配手机 手机友好 自适应' },
  { zh: '表单', en: 'Form', slug: 'form', cat: '前端', say: '帮我做个注册页，用户填完信息点提交那种。', alias: '填写 注册 提交 输入' },
  // 后端 3
  { zh: 'API 接口', en: '', slug: 'api', cat: '后端', say: '就是两个系统之间的传话通道，按规则说就能拿到想要的数据。', alias: '接口 对接 前后端通信 webhook' },
  { zh: '数据库', en: 'Database', slug: 'database', cat: '后端', say: '用户填的东西得存下来，下次打开还在，不能刷新就没了。', alias: '存储 存数据 表 数据库索引 持久化' },
  { zh: '认证', en: 'Authentication', slug: 'auth', cat: '后端', say: '要登录才能进自己的页面，没登录的跳到登录页。', alias: '登录 权限 鉴权 token 会话' },
  // AI·Agent 5
  { zh: '大模型', en: 'LLM', slug: 'llm', cat: 'AI·Agent', say: '就是背后那个会读心又会给代码的 AI 大脑，换脑子回答风格不一样。', alias: '模型 模型选择 chatgpt claude deepseek gemini ai 大脑' },
  { zh: '提示词', en: 'Prompt', slug: 'prompt', cat: 'AI·Agent', say: '告诉 AI 你要什么、怎么做的指令，写得越清楚，回答越靠谱。', alias: '指令 描述 说法 话术' },
  { zh: '上下文', en: 'Context', slug: 'context', cat: 'AI·Agent', say: 'AI 得知道你前面说了什么、项目长什么样，才不会答非所问。', alias: '记忆 上文 背景信息 上下文窗口' },
  { zh: 'Token', en: '', slug: 'token', cat: 'AI·Agent', say: 'AI 按字收费按字记事，太长会截断，这个计数单位就是 token。', alias: '字数 计费 截断 额度' },
  { zh: '智能体', en: 'Agent', slug: 'agent', cat: 'AI·Agent', say: '让它自己去查资料、写代码、跑测试，一步不用我盯着。', alias: '自动化 ai 干活 工作流 代理' },
  // 技术栈 3
  { zh: '框架与库', en: 'Framework & Library', slug: 'framework', cat: '技术栈', say: '别每次从零搭脚手架，用成熟的地基，别人踩过的坑别再踩。', alias: 'react vue nextjs 库 包 npm 脚手架' },
  { zh: '静态站点与服务器渲染', en: 'SSG / SSR', slug: 'ssg-ssr', cat: '技术栈', say: '内容提前烤好还是现烤？打开速度、SEO、能不能动态，全看这个选择。', alias: '渲染方式 静态 动态 服务端 预渲染' },
  { zh: '部署与托管', en: 'Deploy & Hosting', slug: 'deploy', cat: '技术栈', say: '做完的网站要放到公网让人访问，还要有个域名。', alias: '上线 发布 域名 vercel cloudflare 服务器' },
  // Git 2
  { zh: '仓库与提交', en: 'Repo & Commit', slug: 'repo-commit', cat: 'Git', say: '代码要能存档，改坏了能回到上一个存档点。', alias: '存档 git 版本 历史 回滚' },
  { zh: '分支', en: 'Branch', slug: 'branch', cat: 'Git', say: '新功能先在分叉上做，做好了再合回主线，别把主线弄坏。', alias: '合并 merge 主线 分叉 pull request pr' },
  // 产品与设计 3
  { zh: 'MVP', en: '', slug: 'mvp', cat: '产品与设计', say: '先做一个能用的最小版本上线试试，别一上来就想做全能产品。', alias: '最小可用 第一版 先上线 小步快跑' },
  { zh: '用户流程', en: 'User Flow', slug: 'user-flow', cat: '产品与设计', say: '从进来第一步到完成目标，每一步点什么、跳到哪，先画清楚。', alias: '路径 步骤 转化 跳转' },
  { zh: '线框图', en: 'Wireframe', slug: 'wireframe', cat: '产品与设计', say: '先别管好不好看，把页面上有哪几块、摆在哪定下来。', alias: '草图 原型 布局 排版' },
];
  ...
  function render() {
    ...
    var html = list.length ? list.map(function (t) {
      ...
      return '<a class="t-item" href="term-detail.html?slug=' + t.slug + '">' +
        ...
        '<div class="meta"><span class="tag">' + t.cat + '</span></div></a>';
