...
export function ConceptHero({ slug }: { slug: string }) {
  ...
  return <figure ref={ref} className={styles.hero} data-kind={slug} aria-label={slug === "tools" ? "请求交给工具，实际结果返回模型" : slug === "context" ? "任务、日志和要求组合成本轮输入" : "从语法错误到检查通过的三次修正"}>
    <div key={replay} className={styles.art} aria-hidden="true">
      {slug === "tools" ? <div className={styles.dispatch}>
