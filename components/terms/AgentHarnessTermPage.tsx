...
function Citation({ id }: { id: string }) {
  return <sup className="vp-citation">{harnessSources.map((source, index) => source.citations.includes(id) &&
    <a key={source.url} href={`#ref-${index + 1}`} aria-label={`参考文献 ${index + 1}：${source.title}`}>[{index + 1}]</a>
  )}</sup>;
}
    ...
    <div className="vp-page-layout">
      ...
      <div className="vp-reading-content">
        ...
        <section className="vp-chapter" id="need"><div className="vp-chapter-content">
          ...
          <p id="cite-terminology" className="vp-citation-target">阅读其他资料时，会看到“框架”“运行时”和“Harness”有重叠。LangChain 的一种分法是：框架提供搭建智能体的组件，运行时处理持久化、恢复等执行问题，Harness 则把工具、指令和默认行为配成一套可用的系统。本文侧重这套系统在运行中做什么；这些名称并没有统一的严格边界。<Citation id="cite-terminology" /></p>
          <details className={styles.supplement} id="name">
            ...
            <p id="cite-word" className="vp-citation-target">Harness 原指马具、系带一类连接装置。借这个比喻：模型提供能力，Harness 把它接到工具和环境上，并约束操作范围。<Citation id="cite-word" /></p>
            </div>
          </details>
        </div></section>
        ...
        <section className="vp-chapter" id="boundary"><div className="vp-chapter-content">
          ...
          <p id="cite-context" className="vp-citation-target"><Term slug="context">上下文</Term>是模型这次生成回复时能参考的信息，包括任务、<Term slug="prompt">指令</Term>、对话记录和工具结果。工具在外部读到日志后，需要把结果加入后续调用，模型才能依据它继续判断。<Citation id="cite-context" /></p>
          ...
          <details className={styles.supplement} id="practice">
            ...
            <div className="vp-disk-story">
              <div className="vp-disk-round">
                <div id="cite-df" className="vp-disk-request vp-citation-target"><h3>先查哪块磁盘满了</h3><code>df -h</code><p>工具返回：/data 已用 99%。<Citation id="cite-df" /></p></div>
              ...
              <div className="vp-disk-round">
                <div id="cite-du" className="vp-disk-request vp-citation-target"><h3>再查谁占得最多</h3><code>du -sh /data/*</code><p>模型根据上一轮结果，继续定位目录。<Citation id="cite-du" /></p></div>
