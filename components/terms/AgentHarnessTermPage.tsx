...
export function AgentHarnessTermPage({ related }: BespokeTermPageProps) {
  ...
  return (
    <main className="vp-concept" id="main-content">
      ...
      <section className="vp-chapter" id="why"><div className="vp-chapter-content"><h2>{harnessSectionTitles.why}</h2>
      ...
      <section className="vp-chapter" id="need"><div className="vp-chapter-content"><h2>{harnessSectionTitles.need}</h2>
      ...
      <section className="vp-chapter" id="boundary"><div className="vp-chapter-content"><h2>{harnessSectionTitles.boundary}</h2><p>回到修服务的例子。模型准备查看 <code>server.log</code>，可能输出一条结构化请求。结构化的意思是：把工具名称和参数按约定的格式写出来，让程序容易识别。</p><figure className="vp-terminal"><figcaption>模型输出的工具请求 <span>简化示意，不是某个 API 的固定格式</span></figcaption><pre><code>{`{
  "tool": "read_file",
  "arguments": {
    "path": "server.log"
  }
