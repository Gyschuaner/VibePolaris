...
export function ConceptGraph({ nodes: initialNodes, edges, categories, variant = "page", centerSlug }: {
  ...
  useEffect(() => {
    const engine = createGraphSimulation(initialNodes, edges, { centerSlug });
  ...
  }, [initialNodes, edges, paintPositions, centerSlug]);

  function releaseNode() {
    const engine = simulation.current;
    if (!engine) return;
    const node = engine.nodes().find(item => item.slug === gesture.current?.slug);
    if (!node || node.slug === centerSlug || node.fx == null && node.fy == null) return;
  ...
  const frame = useCallback((items: GraphNode[], detail = false) => {
    ...
    const availableWidth = width - (!inline && detail && width > 760 ? 350 : 0);
    const availableHeight = height - (inline && detail ? 92 : detail && width <= 760 ? Math.min(290, height * .43) : 0);
    ...
    const middleX = inline && !detail ? 0 : (minX + maxX) / 2;
    const middleY = inline && !detail ? 0 : (minY + maxY) / 2;
    const spanX = 2 * Math.max(maxX - middleX, middleX - minX);
    const spanY = 2 * Math.max(maxY - middleY, middleY - minY);
    const padding = inline ? 48 : 100;
    const margin = inline ? 80 : 160;
    const scale = Math.max(.12, Math.min(detail ? 1.5 : 1, (availableWidth - padding) / (spanX + margin), (availableHeight - padding) / (spanY + margin)));
    setView({ x: availableWidth / 2 - middleX * scale, y: availableHeight / 2 - middleY * scale, scale });
    setReframing(true);
  }, [inline]);
  ...
  function moveDrag(event: PointerEvent<HTMLDivElement>) {
    ...
    if (start.slug && start.point) {
      ...
      if (!node || node.slug === centerSlug) return;
