...
function MiniPage({ enhanced, compact = false }: { enhanced?: boolean; compact?: boolean }) {
  return (
    <div className={`mini-page${enhanced ? " is-enhanced" : ""}${compact ? " is-compact" : ""}`} aria-hidden="true">
      ...
      <div className="term-story-shell">
        ...
        {isCss ? (
          <section className="term-explainer" aria-labelledby="term-explainer-heading">
            ...
            <div className="term-preview-stage" aria-live="polite">
              <CssPreview mode={previewMode} />
            </div>
            <p className="term-preview-caption">
              {previewMode === "plain" && "结构没有变化，所以两边看起来一样。"}
              {previewMode === "styled" && "同一份内容，通过 CSS 获得层级、氛围和清晰的阅读路径。"}
